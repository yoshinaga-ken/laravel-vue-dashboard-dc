...
In-range pnpm updates only. Do not bump Tailwind 4, Vite 8, laravel-vite-plugin 3, Vitest 5, ESLint 10, Apollo Client 4, GraphQL 17, jQuery 4, or Vuetify 4 unless the user asks for a dedicated upgrade. Keep Inertia on v3 (`@inertiajs/vue3` + `inertiajs/inertia-laravel`) and VueUse on v14 (`@vueuse/core`). Pin vue-tsc to ~3.2.x; 3.3.x fails type-check with TypeScript 6 baseUrl deprecation (TS5101). This project does not use Inertia SSR or `@inertiajs/vite`; keep laravel-vite-plugin page resolution.
