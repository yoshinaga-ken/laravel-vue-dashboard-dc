...
export function loadScriptJQueryUIDatepickerJa(base) {
  return loadScriptCss('jquery-ui/ui/i18n/datepicker-ja-ex.js', null, base)
}

export async function loadScriptDataTables(base) {
  if ($.fn.dataTables === undefined) {
    await loadScriptCss('mark.js/dist/jquery.mark.min.js', null, base)

    await loadScriptCss('datatables/datatables.min.js', 'datatables/datatables.min.css', base)
...
export function loadScriptSparkline(base) {
  // $.fn.sparkline
  return loadScriptCss('jquery-sparkline/dist/jquery.sparkline.min.js', null, base)
}
...
export async function loadScriptFiler(base) {
  ...
  if ($.fn.dialogExtend === undefined) {
    await loadScriptCss('jquery-dialogextend/build/jquery.dialogextend.min.js', null, base)
  }

  const isFsFilerDlg = window.FsFilerDlg && typeof window.FsFilerDlg === 'function'
  if (!isFsFilerDlg) {
    await loadScriptCss('fsFilerDlg.js', null, base)
  }

  if ($.fn.fs_filer === undefined) {
    await loadScriptCss('ch_common.js', 'ch_common.css', base)
  }
}
