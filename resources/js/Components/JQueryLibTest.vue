<template>
  ...
  <div id="blue" style="width: 300px;height: 100px;background: blue;">
    draggable
    <button class="button-cls" @click="onClickM">Click</button>
  </div>
  ...
  <div class="dark">
    <div class="dc_panel">
      <div class="bg-theme-col text-theme-col">
        bg-theme-col text-theme-col
      </div>
      <div class="bg-theme-col2 text-theme-col2">
        bg-theme-col2 text-theme-col2
      </div>
    </div>
  </div>

  <div>
    <h2>福岡県COVID-19感染者数</h2>
...
<script setup>
/**
 * Dc.jsのグラフの方が、AIで作成補助してもらった
 *  @see https://claude.ai/chat/51fc3c78-e224-4f40-b5e7-332c237923ba
 * ご要望のグラフを作成するために、Vue 3とdc.jsを使用したコンポーネントを作成します
 * ベースとしたサンプル
 *  @see barchart&piechart sample
 *    https://dc-js.github.io/dc.js/examples/adjustable-threshold.html
 *    http://localhost:8888/web/examples/adjustable-threshold.html
 *    https://localhost/jslib/dc.js/web-src/examples/adjustable-threshold.html
 */
...
//import {toRomaji} from 'wanakana'; //CODE_WANAKANA
...
const BASE = computed(() => {
  return import.meta.env.DEV ? '/' : import.meta.env.VITE_DOCUMENT_ROOT;
});
...
const createAdjustableThresholdPage = () => {
  var chart = new dc.BarChart("#test"),
    pie = new dc.PieChart('#pie');
  d3.csv(BASE.value + "morley.csv").then(function (experiments) {
    ...
    var ndx = crossfilter(experiments),
      runExptDim = ndx.dimension(function (d) {
        return multikey(d.Run, d.Expt);
      }),
...
onMounted(async () => {
  ...
  // await loadScriptVectormap(); CODE_VECTOR_MAP
  ...
  // console.log(toRomaji('ローマストリング23'));//CODE_WANAKANA

  const csvData = await d3.csv(BASE.value + 'covid19-fukuoka-2020-03.csv');
  ...
  const data = csvData.map(d => ({
    date: d3.timeParse('%Y-%m-%d')(d.日付),
    gender: d.性別,
    count: +d.感染者数
  }));

  // Crossfilterインスタンスの作成
  const ndx = crossfilter(data);

  // ディメンションとグループの作成
  const dateDim = ndx.dimension(d => d.date);
  const genderGroup = dateDim.group().reduce(
    (p, v) => {
      p[v.gender] = (p[v.gender] || 0) + v.count;
      return p;
    },
    (p, v) => {
      p[v.gender] = (p[v.gender] || 0) - v.count;
  ...
  // チャートの設定
  // const chart = dc.barChart(chartRef.value);
  ...
  chart
    ...
    .xAxisLabel('日付')
    .dimension(dateDim)
    .group(genderGroup, '男性', d => d.value['男性'] || 0)
    .stack(genderGroup, '女性', d => d.value['女性'] || 0)
    .title(d => {
      return `${d.key.toISOString().slice(0, 10)}\n男性: ${d.value['男性'] || 0}\n女性: ${d.value['女性'] || 0}`;
    })
    .renderLabel(true)
    .xAxis().ticks(8).tickFormat(d3.timeFormat('%Y-%m-%d'));
  ...
  chart.on('pretransition', function (chart) {
    chart.selectAll('rect.bar')
      ...
      .on('click', dc.d3compat.eventHandler(function (d) {
        ...
        // chart.filter(multikey(d.x, d.layer));
        // dc.redrawAll();
        // chart.filter(d.layer).redrawGroup();//追加選択

        // @see P:\CodeIgniter\application\views\covid19.php
        chart.filter(d.data.key).redrawGroup();//追加選択
        //chart.filter(multikey(d.x, d.layer));//子供項目選択

        // function barChartRedrawGroup(_chart) {
        //
        ...
        // }
        //
        // barChartRedrawGroup(chart);
      }));
  });

  // パイチャートの設定
  // 性別ディメンションとグループの作成
  const genderDim = ndx.dimension(d => d.gender);
  const genderTotalGroup = genderDim.group().reduceSum(d => d.count);

  //const pieChart = dc.pieChart(pieChartRef.value);
  const pieChart = dc.pieChart('#pie-chart');
  pieChart
    .width(300)
    .height(300)
    .dimension(genderDim)
    .group(genderTotalGroup)
    .label(d => `${d.key}: ${d.value}`)
    .title(d => `${d.key}: ${d.value} (${(d.value / ndx.groupAll().reduceSum(d => d.count).value() * 100).toFixed(2)}%)`)
    .legend(dc.legend());

  dc.renderAll();
  // chart.render();
  // pieChart.render();
  createAdjustableThresholdPage();
});
