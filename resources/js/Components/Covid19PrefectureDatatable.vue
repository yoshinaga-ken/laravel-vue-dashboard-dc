...
import { php_number_format } from '@/Utils/phpjs.js'
  ...
  for (var i = 0; i < bar_stacks.length; i++) {
    ...
    o.sparkline(bar_stacks[i], {
      ...
      tooltipFormatter(sparkline, options, f) {
        ...
        let ymd = moment(props.data.spk.max_ymd)
          .subtract(bar_stacks[0].length - (f[0].offset + 1), 'days')
          .format('M/D(ddd)')
        return (
          ymd +
          ' - ' +
          ...
          (f[0].value == 0
            ...
            : '<span class="square" style="background:' +
              ...
              '名<br />') +
          '計: ' +
          (f[4].value + f[3].value + f[2].value + f[1].value + f[0].value) +
          '名<br />'
        )
      },
    })
    ...
    if (mode !== 1) {
      ...
      o.sparkline(props.data.spk.out_of_100k[i], {
        ...
        tooltipFormatter(sparkline, options, f) {
          ...
          return (
            '<span class="ui-icon ui-icon-chart-line" style="background:#FFF;color:' +
            f.color +
            '"></span>10万人中 <span style="' +
            style +
            '">' +
            f.y +
            '</span>名<br />─【累計】─────<br />'
    ...
    o.sparkline(props.data.spk.line[0][i], {
      ...
      tooltipFormatter(sparkline, options, f) {
        ...
        return (
          '感染者: ' +
          php_number_format(f.y) +
          '名<br />' +
          '<span class="ui-icon ui-icon-chart-line" style="background:#FFF;color:' +
          f.color +
          '"></span>死亡: ' +
          php_number_format(death) +
          '名<br />'
        )
      },
    })
        ...
        return (
          ...
          (f[0].value === 1000
            ...
            : f[0].value === 999
              ? '※緊急事態宣言 解除'
              : f[0].value === 998
                ? props.data.chartDateLineYmdMsg[5][1]
                : f[0].value === 997
                  ? props.data.chartDateLineYmdMsg[6][1]
                  : '')
        )
      },
    })
...
const initPrefTableData = () => {
  ...
  for (var i = 0; i < props.data.spk.bar_stacks.length; i++) {
    ...
    spn
      ...
      .css({
        background:
          ...
          '%,#FFFFFF ' +
          p_per +
          '%, #FFFFFF) center center / 100% 30% no-repeat',
      })
    ...
    spn
      ...
      .css({
        background:
          'linear-gradient(to right, #1f77b4 ' +
          per +
          '%, #FFFFFF ' +
          per +
          '% ' +
          (100 - per) +
          '%) center center / 100% 30% no-repeat',
      })
...
const initPrefTable = () => {
  ...
  mm.tbl_pref = $('#tbl_pref').DataTable({
        ...
        render: function (data, type, row, meta) {
          ...
          if (type === 'display') {
            if (props.data.prefTable.link[meta.row] && props.data.prefTable.link[meta.row].length) {
              ...
              let icon = PREFECTURES_EN[data]
                ? '<img src="/img/japan/' + PREFECTURES_EN[data] + '.gif">'
                : ''
        ...
        render: function (data, type) {
          ...
          return type === 'display'
            ? data === '0' || data === ''
              ? ''
              : data > 0
                ? '<span style="color:orange">▲' + Math.abs(data) + '</span>'
                : '<span style="color:green">▼' + Math.abs(data) + '</span>'
            : type === 'sort' && data === ''
              ? 10000
              : data
        },
    ...
    initComplete: function () {
      ...
      if (flt_txt.length && flt_txt.length !== props.data.prefTable.link.length)
        $('#tbl_flt').val(flt_txt.join(' '))
...
<template>
  <div class="container_tbl_pref bg-theme-col text-theme-col">
      ...
      >
        <thead>
          ...
          >
            ...
            <!-- chk_tbl_detail -->
            <th>
              <i class="fa fa-area-chart"></i>感染推移&nbsp;&nbsp;&nbsp;<label
                for="chk_tbl_spkflt"
                id="chk_tbl_spkflt_l"
              ></label
              ><input type="checkbox" id="chk_tbl_spkflt" />
            </th>
          </tr>
        </thead>
        <tbody>
