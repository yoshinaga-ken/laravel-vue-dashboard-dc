...
const title = computed(() => {
  ...
  return (
    msg +
    ' User\n\n' +
    `followers(${followersUserNamesCount}):` +
    followersUserNames.value.join(', ') +
    '\n' +
    `following(${followingUserNamesCount}):` +
    followingUserNames.value.join(', ')
  )
})
</script>
...
<template>
  <button
    ...
    :class="[
      user.is_followed_by ? 'bg-red-200' : 'bg-gray-200',
