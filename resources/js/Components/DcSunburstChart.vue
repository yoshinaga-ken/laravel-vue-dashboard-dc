  ...
  try {
    ...
    dimension = props.ndx.dimension(function (d) {
      ...
      // keyIndexが文字列の場合は書式指定として処理
        ...
        const allEmpty = placeholderValues.every(
    ...
    if (legendConfig.isShow) {
      chart.legend(dc.legend().x(legendConfig.x).y(legendConfig.y))
    }
  ...
  resizeObserver = new ResizeObserver(entries => {
    for (const entry of entries) {
      ...
      if (width > 0 && height > 0) {
        ...
        if (chart && !isResizing) {
          isResizing = true // リサイズ中フラグを立てる
          chart.width(chartWidth.value).height(chartHeight.value)
